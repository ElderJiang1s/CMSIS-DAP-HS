...
#define PAGE_SIZE        EPB_MAX_PKT_SIZE
...
//        HSUSBD_MemCopy(&g_u8OutBuff[g_u32BytesInPageBuf], pu8EpBuf, u32Size);
...
uint8_t usbd_hid_process(void)
{
	...
	{
		//DAP_ProcessCommand(&g_u8OutBuff[g_u32BytesInPageBuf],&g_u8PageBuff[USB_ResponseIn]);
		DAP_ProcessCommand(g_u8OutBuff,g_u8PageBuff);

		// Update request index and flag
		n = USB_RequestOut + 1;
		if(n == EPB_MAX_PKT_SIZE)
			n = 0;
		USB_RequestOut = n;
...
void HID_GetOutReport(uint8_t *EpBuf, uint32_t len)
{
 ...
 //HSUSBD_MemCopy(&g_u8OutBuff[g_u32BytesInPageBuf], EpBuf, len);

	USB_RequestIn++;
	if(USB_RequestIn == EPB_MAX_PKT_SIZE)
		USB_RequestIn = 0;
...
//						g_u32BytesInPageBuf--;
		...
		USB_ResponseOut++;
		if (USB_ResponseOut == EPB_MAX_PKT_SIZE)
			USB_ResponseOut = 0;
